...
import { awsDependencies } from '../lib/delivery/aws.js';
import { canonical } from '../lib/delivery/contract.js';
import { createReconciler } from '../lib/delivery/reconciler.js';
...
test('real SDK clients use fixed AWS endpoints and bounded read operations only', async () => {
    ...
    assert.equal(await deps.command(delivery.configurationId), undefined);
    await deps.invocation(delivery.configurationId, delivery.scope.providerInstanceId);
    ...
    assert.ok(requests.some(r => r.body.includes('PluginName') && r.body.includes('operateConfiguration')));
});
test('reconciler rechecks source history and uses a stable recovery name under duplicate events', async () => {
    const f = fixture(); Object.assign(f.state.execution, { status: 'FAILED' });
    const requests: { name: string; input: any }[] = [];
    const run = createReconciler({ machineArn: settings.machineArn, recoveryVersionArn: settings.machineArn + '-recovery:1' },
    ...
    assert.deepEqual(starts[0]?.input, { stateMachineArn: settings.machineArn + '-recovery:1',
        name: `cleanup-configuration-prepare-${delivery.configurationId}`, input: canonical({ sourceExecutionArn: f.event.executionArn }) });
    await assert.rejects(run({ source: 'aws.states', detail: { ...event.detail, stateMachineArn: 'unrelated' } }), /^Error: configuration_reconciliation_unavailable$/);
});
