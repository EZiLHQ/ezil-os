...
import { receiptFor, type HostAction } from '../lib/mount/contract.js';
...
async function run(options: { lostStart?: boolean; revoke?: boolean; recovery?: boolean;
    ...
    const f = fixture(), delivery = f.input, helper = createMountHelper(settings, f.deps);
    if (options.recovery) Object.assign(f.state.execution, { status: 'ABORTED' });
    const definition = mountDefinition('helper', options.recovery);
    ...
    f.deps.invocation = async id => {
        ...
        return { CommandId: id, InstanceId: delivery.work.authorization.scope.providerInstanceId, DocumentName: settings.documentName,
            DocumentVersion: settings.documentVersion, PluginName: 'operateMount', Status: 'Success', ResponseCode: 0,
            StandardOutputContent: JSON.stringify({ schemaVersion: 1, authorizationId: delivery.work.authorization.authorizationId,
                scope: delivery.work.authorization.scope, status, ...(status === 'succeeded'
...
test('full Standard graph delivers, observes and emits only exact mounted receipt', async () => {
...
test('revocation during work and terminal workflow interruption use host cancellation fence', async () => {
    const revoked = await run({ revoke: true }); assert.equal(revoked.error, 'MountCancelled');
...
test('lost/forged command evidence and wrong receipts cannot produce success', async () => {
    for (const options of [{ missing: true }, { forged: true }, { wrongReceipt: true }]) {
        const r = await run(options); assert.equal(r.result, undefined); assert.equal(r.error, 'MountUnconfirmed');
...
test('CDK binds pinned documents, Standard history, restricted IAM and disabled reconciliation', () => {
    for (const recovery of [false, true]) {
        const definition = mountDefinition('helper', recovery), reached = new Set<string>(), pending = [definition.StartAt];
    ...
    new ComputerMountDelivery(stack, 'Configuration', { settings: config, machineName: 'ezil-mount-pilot', authorityKeyArn: settings.deployment.dataKeyArn });
