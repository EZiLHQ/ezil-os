...
import { deliveryDefinition } from '../lib/delivery/definition.js';
import { resultFor, type HostAction } from '../lib/delivery/contract.js';
...
async function run(options: { lostStart?: boolean; revoke?: boolean; recovery?: boolean;
    ...
    const definition = deliveryDefinition('helper', options.recovery);
    ...
    f.deps.invocation = async id => {
        ...
        return { CommandId: id, InstanceId: delivery.scope.providerInstanceId, DocumentName: settings.documentName,
            DocumentVersion: settings.documentVersion, PluginName: 'operateConfiguration', Status: 'Success', ResponseCode: 0,
            StandardOutputContent: JSON.stringify({ schemaVersion: 1, configurationId: delivery.configurationId,
                scope: delivery.scope, operation: delivery.operation, status, ...(status === 'succeeded'
                    ? { result: options.wrongReceipt ? { loaded: true } : resultFor(delivery) } : {}) }) };
    };
...
test('full Standard graph delivers, observes and emits only exact preparation receipt', async () => {
    const r = await run(); assert.deepEqual(r.result, resultFor(delivery)); assert.equal(r.startCount, 1);
...
test('accepted start with lost AWS response is recovered through observation, never restarted', async () => {
    const r = await run({ lostStart: true }); assert.deepEqual(r.result, resultFor(delivery)); assert.equal(r.startCount, 1);
});
test('revocation during work and terminal workflow interruption use host cancellation fence', async () => {
    const revoked = await run({ revoke: true }); assert.equal(revoked.error, 'ConfigurationCancelled');
...
test('lost/forged command evidence and wrong receipts cannot produce success', async () => {
    for (const options of [{ missing: true }, { forged: true }, { wrongReceipt: true }]) {
        const r = await run(options); assert.equal(r.result, undefined); assert.equal(r.error, 'ConfigurationUnconfirmed');
...
test('CDK binds pinned documents, Standard history, restricted IAM and disabled reconciliation', () => {
    for (const recovery of [false, true]) {
        const definition = deliveryDefinition('helper', recovery), reached = new Set<string>(), pending = [definition.StartAt];
    ...
    const app = new App(), stack = new Stack(app, 'DeliveryTest', { env: { account: settings.accountId, region: settings.region } });
    const { machineArn: _machine, documentName: _name, ...config } = settings;
    new ConfigurationDelivery(stack, 'Configuration', { settings: config, machineName: 'ezil-delivery-pilot', authorityKeyArn: settings.dataKeyArn });
