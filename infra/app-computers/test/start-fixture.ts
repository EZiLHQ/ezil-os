import { canonical, type Settings, type StartInput } from '../lib/start/contract.js';
import type { Dependencies } from '../lib/start/aws.js';
import type { HelperEvent } from '../lib/start/helper.js';
export const settings:Settings={deployment:{accountId:'123456789012',region:'us-east-1',availabilityZone:'us-east-1a',
    ...
    controlKeyPolicy:{accountId:'123456789012',region:'us-east-1',namespace:'pilot',controlDomain:'control.ezil.org',
        kmsKeyArn:'arn:aws:kms:us-east-1:123456789012:key/bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbbb'},
    bucket:'ezil-test-configurations',machineArn:'arn:aws:states:us-east-1:123456789012:stateMachine:ezil-start-pilot',workflowVersion:'1',
    authorityOrigin:'https://cloud.ezil.org',authoritySecretArn:'arn:aws:secretsmanager:us-east-1:123456789012:secret:ezil/start-abcdef',
    documentName:'ezil-start-pilot-test',documentVersion:'1',documentHash:'a'.repeat(64)};
export function fixture() {
    ...
    const configuration={configurationId:'44444444-4444-4444-8444-444444444444',revision:1,digest:'c'.repeat(64),bytes:1024};
    const input:StartInput={schemaVersion:1,work:{schemaVersion:1,authorizationId:'11111111-1111-4111-8111-111111111111',
        mountAuthorizationId:'66666666-6666-4666-8666-666666666666',scope,configuration,
        controlKey:{versionId:'77777777-7777-4777-8777-777777777777',policy:settings.controlKeyPolicy,
            secretArn:`arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/computers/${scope.computerId}/generations/1/control-abcdef`},
        issuedAt,expiresAt:issuedAt+300,deployment:{...settings.deployment,
            instanceProfileArn:`arn:aws:iam::123456789012:instance-profile/ezil/pilot/computers/${scope.computerId}/g1`}},
        configuration:{schemaVersion:1,operation:'prepare',scope,configurationId:configuration.configurationId,revision:configuration.revision,
            digest:configuration.digest,object:{bucket:settings.bucket,
                key:`pilot/computers/${scope.computerId}/generations/1/configurations/${configuration.configurationId}.json`,
                versionId:'immutable-version',sha256:configuration.digest,bytes:configuration.bytes}}};
    const state={now:start+1000,authorized:true,writer:true,authorityCalls:0,execution:{
        executionArn:settings.machineArn.replace(':stateMachine:',':execution:')+':start-'+input.work.authorizationId,
        stateMachineArn:settings.machineArn,stateMachineVersionArn:settings.machineArn+':1',name:'start-'+input.work.authorizationId,
...
export function providerFixture(input:StartInput) {
    const d=input.work.deployment,s=input.work.scope;
