...
import { awsDependencies } from '../lib/start/aws.js';
import { canonical } from '../lib/start/contract.js';
import { createStartReconciler } from '../lib/start/reconciler.js';
import { fixture, settings } from './start-fixture.js';

// A real client preserves the SDK paginator's instanceof contract; sends stay local.
function fakeClient(send: (command: any) => Promise<any>) {
    const client = new SFNClient({ region: 'us-east-1' });
    client.send = send as SFNClient['send']; return client;
}
...
test('real SDK clients use fixed AWS endpoints and bounded read operations only', async () => {
    ...
    assert.equal(await deps.command(delivery.work.authorizationId), undefined);
    await deps.invocation(delivery.work.authorizationId, delivery.work.scope.providerInstanceId);
    ...
    assert.ok(requests.some(r => r.body.includes('PluginName') && r.body.includes('operateStart')));
});
test('reconciler rechecks source history and uses a stable recovery name under duplicate events', async () => {
    ...
    const run = createStartReconciler({ machineArn: settings.machineArn, recoveryVersionArn: settings.machineArn + '-recovery:1' },
        fakeClient(async (command: any) => { requests.push({ name: command.constructor.name, input: command.input });
    ...
    assert.deepEqual(starts[0]?.input, { stateMachineArn: settings.machineArn + '-recovery:1',
        name: `cleanup-start-${delivery.work.authorizationId}`, input: canonical({ sourceExecutionArn: f.event.executionArn }) });
    await assert.rejects(run({ source: 'aws.states', detail: { ...event.detail, stateMachineArn: 'unrelated' } }), /^Error: start_reconciliation_unavailable$/);
});
...
test('scheduled scans fail visibly at ten pages, including repeated tokens',async()=>{
    for(const repeated of [false,true]) {
        let pages=0;
        const run=createStartReconciler({machineArn:settings.machineArn,recoveryVersionArn:settings.machineArn+'-recovery:1'},
            fakeClient(async command=>{assert.equal(command.constructor.name,'ListExecutionsCommand');pages++;
                return {executions:[],nextToken:repeated?'repeated':String(pages)};}));
        await assert.rejects(run({source:'aws.events','detail-type':'Scheduled Event'}),/^Error: start_reconciliation_unavailable$/);
        assert.equal(pages,10);
    }
});
test('reconciler rejects redrives and future/invalid history; successful work never starts cleanup',async()=>{
    for(const change of [{redriveCount:1},{startDate:new Date(NaN)},{startDate:new Date(Date.now()+60000)},
        {stateMachineAliasArn:'alias'},{status:'SUCCEEDED'}]) {
        const f=fixture();Object.assign(f.state.execution,{status:'FAILED'},change);let starts=0;
        const run=createStartReconciler({machineArn:settings.machineArn,recoveryVersionArn:settings.machineArn+'-recovery:1'},
            fakeClient(async command=>{if(command.constructor.name==='DescribeExecutionCommand')return f.state.execution;starts++;return {};}),()=>f.state.now);
        const result=run({source:'aws.states',detail:{stateMachineArn:settings.machineArn,executionArn:f.event.executionArn}});
        if(change.status==='SUCCEEDED')await result;else await assert.rejects(result,/start_reconciliation_unavailable/);
