...
import { startDefinition } from './start/definition.js';
import { SettingsSchema, type Settings } from './start/contract.js';

export interface ComputerStartDeliveryProps {
...
export class ComputerStartDelivery extends Construct {
    readonly machine: states.CfnStateMachine;
    readonly version: states.CfnStateMachineVersion;
    constructor(scope: Construct, id: string, props: ComputerStartDeliveryProps) {
        super(scope, id);
        ...
        if (!/^[A-Za-z0-9_-]{1,60}$/.test(props.machineName) || stack.region !== 'us-east-1'
            ...
            || !new RegExp(`^arn:aws:kms:us-east-1:${s.deployment.accountId}:key/[a-f0-9-]{36}$`).test(props.authorityKeyArn)) throw new Error('invalid_start_deployment');
        const source = readFileSync(join(__dirname, '../documents/start.json'), 'utf8');
        const documentName = `ezil-start-${s.deployment.namespace}-${createHash('sha256').update(source).digest('hex').slice(0,16)}`;
        if ('documentName' in s && s.documentName !== documentName) throw new Error('invalid_start_document_name');
        ...
        const helper = new nodejs.NodejsFunction(this, 'Helper', { runtime: lambda.Runtime.NODEJS_24_X,
            entry: join(__dirname, 'start/helper.ts'), handler: 'handler', timeout: Duration.seconds(35), memorySize: 256,
            ...
            environment: { EZIL_START_SETTINGS: JSON.stringify({ ...s, machineArn, documentName }) },
            logGroup: helperLogs });
        ...
        const makeMachine = (suffix: string, recovery: boolean) => {
            ...
            const machine = new states.CfnStateMachine(this, suffix, { stateMachineName: props.machineName + (recovery ? '-recovery' : ''),
                ...
                definitionString: JSON.stringify(startDefinition(helperVersion.functionArn, recovery)),
        ...
        const reaper = new nodejs.NodejsFunction(this, 'Reconciler', { runtime: lambda.Runtime.NODEJS_24_X,
            entry: join(__dirname, 'start/reconciler.ts'), handler: 'handler', timeout: Duration.minutes(2), memorySize: 256,
            ...
            environment: { EZIL_START_MACHINE_ARN: machineArn, EZIL_START_RECOVERY_VERSION_ARN: recoveryVersion.ref } });
