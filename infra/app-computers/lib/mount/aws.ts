...
import { canonical, equal, type MountInput, type Settings } from './contract.js';

export const AUTHORITY_PATH = '/api/internal/computers/mount-authority';
export function signature(body: string, key: string, timestamp: string) {
    if (!/^[a-f0-9]{64}$/.test(key) || !/^[0-9]{10}$/.test(timestamp)) throw new Error('mount_authority_unavailable');
    return createHmac('sha256',Buffer.from(key,'hex')).update(['ezil-mount-authority-v1','POST',AUTHORITY_PATH,
...
export interface Dependencies {
    ...
    authority(input:MountInput):Promise<boolean>;
    writer(input:MountInput):Promise<boolean>;
...
export function awsDependencies(s:Settings,test?:{requestHandler:EC2ClientConfig['requestHandler'];credentials:EC2ClientConfig['credentials'];fetcher:typeof fetch}):Dependencies {
    ...
    return {now:Date.now,execution:arn=>sfn.send(new DescribeExecutionCommand({executionArn:arn}),request()),
        command:async id=>{
            ...
            if(r.NextToken||(r.Commands?.length??0)>1)throw new Error('mount_command_ambiguous');return r.Commands?.[0];
        },
        invocation:async(id,instance)=>{
            try{return await ssm.send(new GetCommandInvocationCommand({CommandId:id,InstanceId:instance,PluginName:'operateMount'}),request());}
            catch(e){if(e instanceof Error&&e.name==='InvocationDoesNotExist')return undefined;throw e;}
        },
        writer:async i=>{
            const scope=i.work.authorization.scope;
        ...
        authority:async i=>{
            ...
            if(v.ARN!==s.authoritySecretArn||v.SecretBinary||!v.SecretString||!v.VersionStages?.includes('AWSCURRENT'))throw new Error('mount_authority_unavailable');
            return requestAuthority(s.authorityOrigin,v.SecretString,i,test?.fetcher);
        },
    };
}
export async function requestAuthority(origin:string,key:string,i:MountInput,fetcher:typeof fetch=fetch) {
...
export function validWriter(input:MountInput,instances:DescribeInstancesResult,volumes:DescribeVolumesResult) {
    const scope=input.work.authorization.scope,d=input.work.deployment;
