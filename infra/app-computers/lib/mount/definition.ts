...
export function mountDefinition(helperArn: string, recovery = false) {
    ...
    return { Comment: 'Versioned mount delivery; a mounted receipt is not application readiness',
        ...
        States: {
            ...
            Cancelled: recovery ? { Type: 'Pass', Result: { status: 'cancelled' }, End: true }
                : { Type: 'Fail', Error: 'MountCancelled', Cause: 'Host cancellation fence and quiescence observed' },
            Unconfirmed: { Type: 'Fail', Error: 'MountUnconfirmed', Cause: 'Delivery outcome or stop could not be confirmed' },
        },
    };
}
