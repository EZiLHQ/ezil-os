...
import { FencedWritersSchema, type ComputerLifecycleIntent, type ComputerRecoveryIntent, type FencedWriters } from './computer-recovery.js';

export interface Dependencies {
    execution(arn:string):Promise<DescribeExecutionOutput>;
    authority(i:LifecycleIntent,digest:string):Promise<boolean>;
    recoveryAuthority?(i:ComputerRecoveryIntent,digest:string):Promise<{writers:FencedWriters}|null>;
...
export function awsDependencies(settings:Settings,test?:{requestHandler:EC2ClientConfig['requestHandler'];credentials:EC2ClientConfig['credentials'];fetcher:typeof fetch}):Dependencies {
    ...
    const authorityResponse=async(i:ComputerLifecycleIntent,digest:string):Promise<unknown>=>{
            ...
            try {let bytes=0;const parts:Uint8Array[]=[];
                for(;;){const p=await reader.read();if(p.done)break;bytes+=p.value.length;if(bytes>(i.schemaVersion===1?4096:32768))throw new Error();parts.push(p.value);}
                if(r.status===403)return null;if(r.status!==200)throw new Error();
    ...
    return {now:Date.now,
        ...
        authority:async(i,digest)=>equal(await authorityResponse(i,digest),{authorized:true,...authorityFor(i,digest)}),
        recoveryAuthority:async(i,digest)=>{
            const response=await authorityResponse(i,digest);
            if(response===null)return null;
            if(!response||typeof response!=='object'||Array.isArray(response))throw new Error('authority_unavailable');
            const {writers,...scope}=response as Record<string,unknown>;
            if(!equal(scope,{authorized:true,...authorityFor(i,digest)}))throw new Error('authority_unavailable');
            return {writers:FencedWritersSchema.parse(writers)};
        },
    };
}
