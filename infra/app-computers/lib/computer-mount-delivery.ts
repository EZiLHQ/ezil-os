...
import { CfnOutput, Duration, RemovalPolicy, Stack, aws_iam as iam, aws_lambda as lambda,
...
import { mountDefinition } from './mount/definition.js';
import { SettingsSchema, type Settings } from './mount/contract.js';

export interface ComputerMountDeliveryProps {
...
export class ComputerMountDelivery extends Construct {
    readonly machine: states.CfnStateMachine;
    readonly version: states.CfnStateMachineVersion;
    constructor(scope: Construct, id: string, props: ComputerMountDeliveryProps) {
        super(scope, id);
        const stack = Stack.of(this), s = props.settings;
        if (!/^[A-Za-z0-9_-]{1,60}$/.test(props.machineName) || stack.region !== 'us-east-1'
            || stack.account !== s.deployment.accountId
            || !new RegExp(`^arn:aws:kms:us-east-1:${s.deployment.accountId}:key/[a-f0-9-]{36}$`).test(props.authorityKeyArn)) throw new Error('invalid_mount_deployment');
        const source = readFileSync(join(__dirname, '../documents/mount.json'), 'utf8');
        const documentName = `ezil-mount-${s.deployment.namespace}-${createHash('sha256').update(source).digest('hex').slice(0,16)}`;
        if ('documentName' in s && s.documentName !== documentName) throw new Error('invalid_mount_document_name');
        ...
        const machineArn = `arn:aws:states:us-east-1:${s.deployment.accountId}:stateMachine:${props.machineName}`;
        SettingsSchema.parse({ ...s, machineArn, documentName });
        ...
        const helper = new nodejs.NodejsFunction(this, 'Helper', { runtime: lambda.Runtime.NODEJS_24_X,
            entry: join(__dirname, 'mount/helper.ts'), handler: 'handler', timeout: Duration.seconds(35), memorySize: 256,
            ...
            environment: { EZIL_MOUNT_SETTINGS: JSON.stringify({ ...s, machineArn, documentName }) },
            logGroup: helperLogs });
        ...
        helper.addToRolePolicy(new iam.PolicyStatement({ actions: ['ec2:DescribeInstances', 'ec2:DescribeVolumes',
            ...
            conditions: { StringEquals: { 'aws:RequestedRegion': s.deployment.region } } })); // These read APIs do not support resource scoping
        ...
        helper.addToRolePolicy(new iam.PolicyStatement({ actions: ['kms:Decrypt'], resources: [props.authorityKeyArn],
            conditions: { StringEquals: { 'kms:ViaService': `secretsmanager.${s.deployment.region}.amazonaws.com`,
        ...
        const makeMachine = (suffix: string, recovery: boolean) => {
            const role = new iam.Role(this, `${suffix}Role`, { assumedBy: new iam.ServicePrincipal('states.amazonaws.com', {
                conditions: { StringEquals: { 'aws:SourceAccount': s.deployment.accountId },
            ...
            conditions: { StringEquals: { 'ssm:resourceTag/ezil:managed-by': 'app-computer-platform',
                'ssm:resourceTag/ezil:stage': s.deployment.namespace } } }));
            ...
            const machine = new states.CfnStateMachine(this, suffix, { stateMachineName: props.machineName + (recovery ? '-recovery' : ''),
                ...
                definitionString: JSON.stringify(mountDefinition(helperVersion.functionArn, recovery)),
        ...
        const reaper = new nodejs.NodejsFunction(this, 'Reconciler', { runtime: lambda.Runtime.NODEJS_24_X,
            entry: join(__dirname, 'mount/reconciler.ts'), handler: 'handler', timeout: Duration.minutes(2), memorySize: 256,
            ...
            environment: { EZIL_MOUNT_MACHINE_ARN: machineArn, EZIL_MOUNT_RECOVERY_VERSION_ARN: recoveryVersion.ref } });
