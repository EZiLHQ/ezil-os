...
export function createReconciler(settings: ReconcilerSettings, client: Pick<SFNClient, 'send'>, now = Date.now) {
    return async (input: unknown) => {
        try {
            ...
            const reconcile = async (arn: string) => {
                ...
                if (!/^configuration-(prepare|reload)-[a-f0-9-]{36}$/.test(e.name ?? '') || arn !== prefix + e.name) throw new Error();
        ...
        } catch { throw new Error('configuration_reconciliation_unavailable'); }
    };
}
let run: ReturnType<typeof createReconciler> | undefined;
export const handler = (event: unknown) => {
    if (!run) {
        const machineArn = process.env.EZIL_DELIVERY_MACHINE_ARN ?? '', recoveryVersionArn = process.env.EZIL_RECOVERY_VERSION_ARN ?? '';
        if (!/^arn:aws:states:us-east-1:\d{12}:stateMachine:[A-Za-z0-9_-]+$/.test(machineArn)
            ...
            throw new Error('configuration_reconciliation_unavailable');
        }
        run = createReconciler({ machineArn, recoveryVersionArn }, new SFNClient({ region: 'us-east-1', maxAttempts: 1,
