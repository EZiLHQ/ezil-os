...
import { canonical, equal, hostObservation, hostOperation, parseInput, receiptFor, SettingsSchema,
    type StartInput, type HostAction, type Settings } from './contract.js';
...
export function commandParameters(settings: Settings, delivery: StartInput, action: HostAction) {
    return { DocumentName: settings.documentName, DocumentVersion: settings.documentVersion,
        ...
        InstanceIds: [delivery.work.scope.providerInstanceId],
        Parameters: { Operation: [hostOperation(delivery, settings, action)] },
        TimeoutSeconds: 30, Comment: `ezil-start-${delivery.work.authorizationId}-${action}`,
    };
}
...
export function createStartHelper(settings: Settings, deps: Dependencies) {
    async function execute(input: unknown): Promise<Decision> {
        ...
        const prefix = settings.machineArn.replace(':stateMachine:', ':execution:') + ':start-';
        ...
        if (execution.redriveCount !== 0 || (event.recovery ? !['FAILED', 'TIMED_OUT', 'ABORTED'].includes(execution.status ?? '')
            : execution.status !== 'RUNNING')) throw new Error('invalid_workflow_execution');
        const delivery = parseInput(JSON.parse(execution.input), settings), a = delivery.work;
        const deadline = a.expiresAt * 1000;
        if (started < a.issuedAt * 1000 || started >= deadline) throw new Error('invalid_workflow_execution');
        if (execution.input !== canonical(delivery) || execution.name !== `start-${a.authorizationId}`
        ...
        if (!cancelling) {
            ...
            catch { cancelling = true; } // loss of authority is not permission to keep starting
        }
        ...
        const uncertain = () => now - attempt.issuedAt < 180000
        ...
        if (invocation.CommandId !== event.commandId || invocation.InstanceId !== delivery.work.scope.providerInstanceId
            || invocation.DocumentName !== expected.DocumentName || invocation.DocumentVersion !== expected.DocumentVersion
            || invocation.PluginName !== 'operateStart') throw new Error('invalid_command_binding');
        ...
        if (status === 'succeeded') {
            // A start receipt is not application readiness. Recheck
    ...
    return async (input: unknown): Promise<Decision> => {
        try { return await execute(input); } catch { throw new Error('start_workflow_unavailable'); }
    };
}

let initialized: ReturnType<typeof createStartHelper> | undefined;
export const handler = (input: unknown) => {
    if (!initialized) {
        try { const settings = SettingsSchema.parse(JSON.parse(process.env.EZIL_START_SETTINGS ?? ''));
            initialized = createStartHelper(settings, awsDependencies(settings)); }
        catch { throw new Error('start_workflow_unavailable'); }
    }
