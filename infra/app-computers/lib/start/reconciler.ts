import { SFNClient, DescribeExecutionCommand, paginateListExecutions, StartExecutionCommand } from '@aws-sdk/client-sfn';
...
export function createStartReconciler(settings: ReconcilerSettings, client: SFNClient, now = Date.now) {
    return async (input: unknown) => {
        try {
            ...
            const reconcile = async (arn: string) => {
                ...
                if (!Number.isFinite(e.startDate.getTime()) || e.startDate.getTime() > now() || e.redriveCount !== 0 || e.stateMachineAliasArn) throw new Error();
                if (!/^start-[a-f0-9-]{36}$/.test(e.name ?? '') || arn !== prefix + e.name) throw new Error();
            ...
            if (input && typeof input === 'object' && 'source' in input && input.source === 'aws.states') {
                ...
                for (const status of ['FAILED', 'TIMED_OUT', 'ABORTED'] as const) {
                    let page = 0;
                    for await (const values of paginateListExecutions({ client, pageSize: 100 }, {
                        stateMachineArn: settings.machineArn, statusFilter: status,
                    }, { abortSignal: AbortSignal.timeout(60000) })) {
                        page++;
                        let old = false;
                        for (const e of values.executions ?? []) {
                            if (!e.executionArn || !e.startDate || !Number.isFinite(e.startDate.getTime()) || e.startDate.getTime() > now()) throw new Error();
                            if (now() - e.startDate.getTime() > 86400000) { old = true; break; }
                            await reconcile(e.executionArn);
                        }
                        if (!values.nextToken || old) break;
                        if (page >= 10) throw new Error(); // bounded scan exhaustion is visible
                    }
                }
            }
            return { status: 'reconciliation_requested' };
        } catch { throw new Error('start_reconciliation_unavailable'); }
    };
}
let run: ReturnType<typeof createStartReconciler> | undefined;
export const handler = (event: unknown) => {
    if (!run) {
        const machineArn = process.env.EZIL_START_MACHINE_ARN ?? '', recoveryVersionArn = process.env.EZIL_START_RECOVERY_VERSION_ARN ?? '';
        if (!/^arn:aws:states:us-east-1:\d{12}:stateMachine:[A-Za-z0-9_-]+$/.test(machineArn)
            ...
            throw new Error('start_reconciliation_unavailable');
        }
        run = createStartReconciler({ machineArn, recoveryVersionArn }, new SFNClient({ region: 'us-east-1', maxAttempts: 1,
