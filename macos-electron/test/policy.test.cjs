...
test('helper ready line strictly verifies version, port and capabilities', () => {
  ...
  assert.equal(readyLine('EZIL_NATIVE_READY ' + JSON.stringify({ contractVersion: 1, port: 1234, capabilities })).port, 1234);
  for (const value of [{ contractVersion: 2, port: 1, capabilities }, { contractVersion: 1, port: 65536, capabilities }, { contractVersion: 1, port: 1234, capabilities: { ...capabilities, isolation: 'container' } }]) assert.throws(() => readyLine('EZIL_NATIVE_READY ' + JSON.stringify(value)));
});
