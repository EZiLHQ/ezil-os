...
app.whenReady().then(async () => {
  ...
  const destroyed = Promise.all([remote, other].map(wc => once(wc, 'destroyed', { signal: AbortSignal.timeout(5000) })));
  await first.close(); await second.close(); await destroyed;
  assert.equal(remote.isDestroyed(), true); assert.equal(other.isDestroyed(), true);
