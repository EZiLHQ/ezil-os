...
const { nativeButtonScript } = require('./product-flow-helpers.cjs');
...
function native(script) { return execFileSync('/usr/bin/osascript', ['-e', script], { encoding: 'utf8', timeout: 45000 }); }
...
async function terminal() {
  ...
  try { await trust.waitFor({ timeout: 2000 }); await trust.click(); } catch {}
...
async function main() {
  ...
  for (let cycle = 0; cycle < 10; cycle++) {
    ...
    report.cycles.push({ cycle: cycle + 1, codeAndBunReadyMs: Date.now() - start, supervisedStopped: owned.length });
...
main().catch(async error => { report.failure = { stage, message: String(error.message).slice(0, 1000) }; if (page && !page.isClosed()) await page.screenshot({ path: path.join(evidence, 'failure.png') }).catch(() => {}); process.exitCode = 1; }).finally(async () => {
  if (app) { if (page && !page.isClosed()) await closeWindow().catch(() => {}); await app.close().catch(() => {}); }
