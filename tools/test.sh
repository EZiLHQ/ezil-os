...
run_supervisor () {
    ...
    if [[ "${1:-}" == "--linux-start-operation" ]]; then
        if [[ "$(uname -s)" != "Linux" || "$(id -u)" != "0" ]]; then
            say "startup operation acceptance requires root in its disposable QEMU fixture"
            return 1
        fi
        node "$TREE/supervisor/test/start-operation.linux.mjs" "${2:-}"
        return $?
    fi
    ...
    if [[ "${1:-}" == "--linux-data-volume" || "${1:-}" == "--linux-retained-host" || "${1:-}" == "--linux-mount-delivery" || "${1:-}" == "--linux-mount-operation" ]]; then
        ...
        if [[ "$1" == "--linux-mount-operation" ]]; then acceptance_script="mount-operation.linux.mjs"; fi
        if [[ "$(id -u)" == "0" ]]; then
            node "$TREE/supervisor/test/$acceptance_script" "${2:-}"
        else
            sudo -n "$(command -v node)" "$TREE/supervisor/test/$acceptance_script" "${2:-}"
        fi
        return $?
    fi
