...
ALL_PACKAGES=(sdk mcp worker app shell tools local infra)
...
run_one () {
    ...
    case "$pkg" in
        ...
        tools)  run_tools  "$@" ;;
        local)  run_local  "$@" ;;
        infra)  typecheck "$TREE/infra/app-computers" npm run typecheck || return 1
                ( cd "$TREE/infra/app-computers" && npm test && npm run synth ) ;;
