...
export async function acquireHostLock(purpose: 'host' | 'preparation' | 'delivery' | 'delivery-management' | 'data-mount' = 'host') {
    await ensureHostDirectory(DIRECTORY);
    if (!['host', 'preparation', 'delivery', 'delivery-management', 'data-mount'].includes(purpose)) throw new Error('host_lock_unavailable');
