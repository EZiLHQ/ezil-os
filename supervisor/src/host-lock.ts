...
export async function acquireHostLock() {
    ...
    const file = await open(`${DIRECTORY}/host.lock`, constants.O_CREAT | constants.O_RDWR | constants.O_NOFOLLOW, 0o600);
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            child.once('exit', code => code === 0 ? resolve() : reject(new Error(code === 73 ? 'host_already_running' : 'host_lock_unavailable')));
        });
