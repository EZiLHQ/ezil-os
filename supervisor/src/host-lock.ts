...
export async function acquireHostLock(purpose: 'host' | 'preparation' | 'delivery' | 'delivery-management' | 'data-mount' | 'data-mount-delivery' | 'mount-management' | 'start-management' = 'host') {
    await ensureHostDirectory(DIRECTORY);
    if (!['host', 'preparation', 'delivery', 'delivery-management', 'data-mount', 'data-mount-delivery', 'mount-management', 'start-management'].includes(purpose)) throw new Error('host_lock_unavailable');
