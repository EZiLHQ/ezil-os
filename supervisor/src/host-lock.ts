...
export async function acquireHostLock(purpose: 'host' | 'preparation' | 'delivery' | 'delivery-management' = 'host') {
    await ensureHostDirectory(DIRECTORY);
    if (!['host', 'preparation', 'delivery', 'delivery-management'].includes(purpose)) throw new Error('host_lock_unavailable');
