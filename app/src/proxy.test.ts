...
it('leaves workflow requests to dedicated authentication without refreshing browser cookies', async () => {
    const request = new NextRequest('https://control.example/api/internal/apps/configuration-authority', {
...
it('preserves normal session refresh on OS and existing API routes', async () => {
    for (const path of ['/os', '/api/trpc/apps.catalog', '/api/internal/apps/configuration-authority/other']) {
