...
it.each(['/api/internal/apps/configuration-authority', '/api/internal/computers/lifecycle-authority'])('leaves %s to dedicated authentication without refreshing browser cookies', async path => {
...
it('preserves normal session refresh on OS and existing API routes', async () => {
    for (const path of ['/os', '/api/trpc/apps.catalog', '/api/internal/apps/configuration-authority/other', '/api/internal/computers/lifecycle-authority/other']) {
        const request = new NextRequest(`https://control.example${path}`);
