...
export const computerLifecycleJobs = pgTable('ezil_computer_lifecycle_jobs', {
    ...
    operation: text('operation').$type<'provision' | 'start' | 'stop' | 'replace' | 'retire' | 'migrate'>().notNull(),
    ...
    check('ezil_computer_lifecycle_jobs_operation_chk', sql`${table.operation} in ('provision', 'start', 'stop', 'replace', 'retire', 'migrate')`),
