 ...
 * credentials. This endpoint reads current lifecycle authority and never sends a
 * provider command, starts compute, or marks an installation successful. */
export function createLifecycleAuthorityHandler(options: {
    ...
    return async (request: Request): Promise<Response> => {
        ...
        if (!LifecycleAuthoritySecretSchema.safeParse(options.secret).success) return response(503, 'lifecycle_authority_unavailable');
        const url = new URL(request.url);
        if (url.pathname !== LIFECYCLE_AUTHORITY_PATH || url.search || url.hash) return response(404, 'not_found');
        ...
        const expected = lifecycleAuthoritySignature(body, options.secret!, timestamp);
