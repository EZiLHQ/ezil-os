...
export const START_AUTHORITY_PATH = '/api/internal/computers/start-authority';
...
export function startAuthoritySignature(body: Uint8Array, secret: string, timestamp: string): string {
    if (!validSecret(secret) || !/^[0-9]{10}$/.test(timestamp)) throw new Error('start_authority_signing_invalid');
    return createHmac('sha256', Buffer.from(secret, 'hex')).update(['ezil-start-authority-v1', 'POST', START_AUTHORITY_PATH,
...
async function bounded<T>(signal: AbortSignal, milliseconds: number, operation: () => Promise<T>, cancel = () => {}) {
    ...
    try {
        if (signal.aborted) throw new Error('start_authority_interrupted');
        const interrupted = new Promise<never>((_resolve, reject) => {
            abort = () => { reject(new Error('start_authority_interrupted')); cancel(); };
 ...
 * issues a grant, starts compute or services, or substitutes for the workflow's
 * provider checks. Supabase cookies/bearers confer no authority on this path. */
export function createStartAuthorityHandler(o: {
    enabled: boolean; secret?: string | undefined; authorize(work: ComputerStartWork): Promise<boolean>;
}) {
    return async (request: Request): Promise<Response> => {
        if (!o.enabled) return response(404, 'not_found');
        if (!validSecret(o.secret)) return response(503, 'start_authority_unavailable');
        const url = new URL(request.url);
        if (url.pathname !== START_AUTHORITY_PATH || url.search || url.hash) return response(404, 'not_found');
        ...
        const expected = startAuthoritySignature(body, o.secret, timestamp);
        ...
        const parsed = ComputerStartWorkSchema.safeParse(input);
        if (!parsed.success) return response(400, 'invalid_request');
        try {
            // The authority implementation refreshes metadata only. Timeout/abort cannot
            ...
            if (request.signal.aborted || !o.enabled) return response(403, 'start_not_current');
            if (!fresh(timestamp)) return response(401, 'unauthorized');
            if (authorized !== true) return response(403, 'start_not_current');
            return Response.json({ authorized: true, work: parsed.data }, { headers: { 'cache-control': 'no-store' } });
        } catch { return response(503, 'start_authority_unavailable'); }
    };
}
