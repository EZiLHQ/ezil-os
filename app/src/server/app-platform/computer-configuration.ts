...
import { appInstallations, appJobs, appOutbox, appRuntimeCommands, appServices, appPortLeases, computerConfigurations,
...
import { compilePreparedInstallation, validatePreparationServiceLease, RuntimePlanError, sameRuntimePlan,
...
export async function produceComputerConfiguration(options: ConfigurationProducerOptions, computerId: string): Promise<ConfigurationResult> {
    ...
    try {
        return await options.database.transaction(tx => produceComputerConfigurationInTransaction(tx, computerId, options.osAccessMode));
    } catch { throw new Error('computer_configuration_unavailable'); }
}

/** Internal composition point for delivery: refresh the exact authorized state
 * and retain all authority locks until the receipt/job transaction commits.
 * The caller owns the transaction and its worker enable gate. */
export async function produceComputerConfigurationInTransaction(
    tx: Parameters<Parameters<typeof db['transaction']>[0]>[0], computerId: string, osAccessMode: OsAccessMode,
    ...
    const authorized = await hasCurrentOsAccess(tx, computer.userId, osAccessMode);
    ...
    if (!config.suspended) {
        ...
        if (installations.length > 128) config.suspended = true;
            ...
            const services = await tx.select().from(appServices).where(and(eq(appServices.installationId, installation.id),
                eq(appServices.computerId, computerId))).for('share');
            const leases = await tx.select().from(appPortLeases).where(and(eq(appPortLeases.installationId, installation.id),
                eq(appPortLeases.computerId, computerId), isNull(appPortLeases.releasedAt))).for('share');
            let prepared: PreparedInstallation;
            try {
                validatePreparationServiceLease({ installationId: installation.id, ...authority, services, leases });
                prepared = compilePreparedInstallation({ installationId: installation.id, ...authority });
            }
            ...
            const [job] = await tx.select().from(appJobs).where(and(eq(appJobs.id, command.jobId),
                ...
                .limit(1).for('share');
            if (!job) continue;
            const plan = await currentStartPlan(tx, command, installation, computer.userId, osAccessMode);
            if (plan) {
