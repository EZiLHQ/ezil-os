// boot-phases.shell.js
//
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ HANDOFF ARTIFACT — destined for `shell/ezil/boot-phases.js`.             ║
// ║                                                                          ║
// ║ It lives here in `app/` only because `shell/` belongs to another agent.  ║
// ║ To adopt it: `git mv` this file to `shell/ezil/boot-phases.js` and       ║
// ║ `src/components/desktop/boot-phases.shell.test.ts` alongside it (dropping ║
// ║ that test's final "drift guard" block, which is the one part that cannot ║
// ║ travel — it imports the TypeScript original). Nothing else changes; there ║
// ║ are no imports to rewrite.                                               ║
// ║                                                                          ║
// ║ This is a MODULE-FORMAT CONVERSION of                                    ║
// ║ `src/components/desktop/boot-phases.ts` and nothing else. Every constant, ║
// ║ every branch and every string is identical; TypeScript types became      ║
// ║ JSDoc `@typedef`s. `boot-phases.shell.test.ts` pins that equivalence     ║
// ║ against the original, input by input, so a "small improvement" to either ║
// ║ copy fails the build.                                                    ║
// ║                                                                          ║
// ║ DO NOT REWRITE THE LOGIC. It already refused to lie during a live        ║
// ║ failure, which is the entire reason it is worth carrying across.         ║
// ╚══════════════════════════════════════════════════════════════════════════╝
//
...
//      `cloudflare-guacamole-provider.ts`, reached from the shell through
//      `POST /api/shell/desktop`), which resolves ONLY at the very end —
//      success or a specific error. Nothing about it is observable mid-flight.
//
...
//      (`withWakeAndOneRetry` in `session.js`) until `WAKE_DEADLINE_MS`. That
//      is not a new signal about the desktop — it says nothing this module may
//      render as progress EARNED — it only means the estimate below gets to
//      keep running honestly instead of being cut short by a dead socket.
//   2. `GET /sandbox/:id/status` (from the shell: `GET /api/shell/desktop`) —
//      cheap, does NOT wake a sleeping container, returns
//      `{ ok, sandboxName, guacamoleRunning, mode }`. Polling it WHILE the
//      preview request is in flight is safe (the container is already being
//      woken by that request) and gives one real mid-boot fact: whether the
//      desktop process has come up.
//
...
// an in-sandbox dev server), and even if it were, it costs 6+ `sandbox.exec()`
// calls and wakes a sleeping container, so it must never be polled
// speculatively. The failure taxonomy below (`BootFailureReason`) instead maps
// the error codes this app's actual boot path (`GuacamolePreviewErrorCode`)
// can genuinely produce, plus a real client-side `timeout`.

/** @typedef {'waking' | 'mounting' | 'starting' | 'connecting'} BootPhaseId */

/**
 * @typedef {object} BootPhaseDef
 * @property {BootPhaseId} id
 * @property {string} label Plain, user-facing label — never internal jargon like "container_start".
 */
 ...
 * not from the happy path. `apps/desktop-window.js` (and its two siblings) ask
 * up to `FRAME_CONFIRM_ATTEMPTS` (3) times, `FRAME_CONFIRM_RETRY_MS` (1.5s)
 * apart, and each ask carries `session.js`'s `STATUS_TIMEOUT_MS` (12s) budget
 * — ~40s if every single one of them times out. Anything below that would fire
 * the deadline BEFORE the caller had finished asking, turning a slow
 * confirmation into a fabricated failure. Measured healthy gap: ~6s.
 */
 ...
 * @returns {BootPhaseId}
 */
export function estimatePhaseForElapsedMs(elapsedMs) {
...
/**
 * @typedef {object} BootProgressState
 * @property {'progress'} kind
 * @property {BootPhaseId} currentPhase
 * @property {boolean} confirmed True only when `currentPhase` is backed by the
 *   real `guacamoleRunning` signal from the status poll — never set from
 *   elapsed time alone.
 * @property {boolean} isRunningLong True once elapsed time is well past the
 *   typical ~22s boot.
 */

/**
 * @typedef {{ kind: 'ready' }} BootReadyState
 * @typedef {{ kind: 'not_configured' }} BootNotConfiguredState
 */
 ...
 *
 * @typedef {'worker_unreachable' | 'sandbox_crashed' | 'desktop_unreachable'
 *   | 'display_not_streaming' | 'timeout' | 'unknown'} BootFailureReason
 */
 ...
 *
 * @typedef {{ kind: 'ready_unverified' }} BootUnverifiedState
 */

/**
 * @typedef {{ kind: 'failed', reason: BootFailureReason }} BootFailureState
 * @typedef {BootProgressState | BootReadyState | BootNotConfiguredState
 *   | BootFailureState | BootUnverifiedState} BootUiState
 */
 ...
 *
 * @typedef {'live' | 'blank' | 'unknown'} DisplayEvidence
 */
 ...
 *
 * @typedef {'bad_request' | 'unauthorized' | 'preconditions_unmet'
 *   | 'custom_domain_required' | 'connection_refused' | 'fetch_failed'
 *   | 'sandbox_runtime_blocked' | 'sandbox_start_failed' | 'sandbox_starting'
 *   | 'worker_http_error' | 'desktop_unreachable' | 'timeout'
 *   | 'unknown'} BootErrorCode
 */
 ...
 */
const DETERMINISTIC_BOOT_ERROR_CODES = [
    'bad_request',
    'unauthorized',
    'preconditions_unmet',
 ...
 * @returns {boolean}
 */
export function isRetryableBootErrorCode(code) {
    if (!code) return true;
    return !DETERMINISTIC_BOOT_ERROR_CODES.includes(code);
}

/**
 * @typedef {object} ComputeBootUiStateInput
 * @property {'not_configured' | 'pending' | 'success' | 'error'} requestStatus
 *   Overall status of the one-shot `POST /sandbox/preview` request.
 * @property {number} elapsedMs ms since the CURRENT PHASE began — not since the
 *   window opened. `pending`: since this boot attempt started, which picks the
 *   phase the time-based estimate highlights. `success`: since the desktop URL
 *   was handed to the browser, a DIFFERENT clock on purpose, so a cold boot
 *   that took 200s does not spend the confirmation's whole budget before the
 *   confirmation has been asked for once. Ignored otherwise.
 * @property {boolean} [confirmedGuacamoleRunning] Last observed value from
 *   polling the cheap `GET /sandbox/:id/status` endpoint while
 *   `requestStatus === 'pending'`. `undefined` means no poll has landed yet —
 *   must NOT be treated as `false` (that would fabricate a negative signal we
 *   don't have).
 * @property {BootErrorCode} [errorCode] Present only when
 *   `requestStatus === 'error'`.
 * @property {boolean} [frameConfirmed] Whether the DESKTOP ORIGIN itself has
 *   been observed answering — not the Worker, and not the iframe's `load`
 *   event. Read only when `requestStatus === 'success'`. 🔴 THREE VALUES,
 *   THREE MEANINGS, same convention as `confirmedGuacamoleRunning` above:
 *   `true` = observed answering, the one and only route to `ready`; `false` =
 *   ASKED AND REFUTED (an error status, or the caller exhausted its own
 *   bounded retries), a real observation and a failure; `undefined` = nobody
 *   has asked yet, which is neither. See the `success` branch.
 */

/**
 * Map a genuine error code to the plain-language failure taxonomy this UI renders.
 * @param {BootErrorCode | undefined} errorCode
 * @returns {BootFailureReason}
 */
 ...
 *
 * @param {ComputeBootUiStateInput} input
 * @returns {BootUiState}
 */
export function computeBootUiState(input) {
    ...
    const currentPhase = confirmedRunning ? 'connecting' : estimated;
 ...
 * away.
 *
 * @param {BootUiState} state the state `computeBootUiState` produced
 * @param {DisplayEvidence | undefined} display what `session.confirmDisplay` observed, if anything
 * @returns {BootUiState}
 */
export function applyDisplayEvidence(state, display) {
...
/** @typedef {'upcoming' | 'current' | 'passed' | 'confirmed'} PhaseVisualState */
 ...
 *
 * @param {BootPhaseId} phaseId
 * @param {BootProgressState} progress
 * @returns {PhaseVisualState}
 */
export function phaseVisualState(phaseId, progress) {
...
/**
 * Classify a caught fetch error from the desktop request's network layer into
 * `BootErrorCode`, adding the one code `GuacamolePreviewErrorCode` doesn't
 * carry: a genuine client-side timeout. `AbortSignal.timeout(...)` rejects
 * with a `DOMException` named `TimeoutError` (verified against Node's `undici`
 * fetch, and the same in every browser) — distinct from a network-level
 * `ETIMEDOUT`, which stays classified as `fetch_failed`.
 *
 * @param {unknown} err
 * @returns {'timeout' | undefined}
 */
export function classifyPreviewFetchError(err) {
...
/**
 * @typedef {object} BootFailureCopy
 * @property {string} title
 * @property {string} body
 */

/** @type {Record<BootFailureReason, BootFailureCopy>} */
export const BOOT_FAILURE_COPY = {
    worker_unreachable: {
...
export const BOOT_NOT_CONFIGURED_COPY = {
 ...
 * @type {BootFailureCopy}
 */
export const BOOT_UNVERIFIED_COPY = {
