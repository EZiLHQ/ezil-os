...
async function boot (viewport = { width: 1440, height: 900 }, { frameOk = true } = {}) {
    ...
    // 🔴 MERGE NOTE (B × W3): open the Browser with the same dock click a real
    // user makes. This suite was written against a base where `boot.js`
    // auto-launched `apps[0]`, so every scenario inherited a desktop window
    // for free; W3 made login open NOTHING (see `boot.js`'s header) and the
    // three scenarios that measure the Browser directly were left measuring a
    // window that never existed — `scenarioStreamFit` threw on a null, and
    // `scenarioCreatedSize` reported `null` rather than a wrong number.
    // `scenarioChrome` only escaped because `leaveFullbleed` happens to click
