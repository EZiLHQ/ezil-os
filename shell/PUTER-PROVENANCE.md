...
|---|---|---|---|---|---|
| `src/UI/UIWindow.js` | 5564 | `src/gui/src/UI/UIWindow.js` (5265) | `5a15719` | 2026-07-31 (import block); 2026-08-02, 2026-08-03, 2026-08-04 (see below) | **Import block, plus small, individually tagged body edits — no longer "nothing else touched", see the note below for why.** Nine imports (login/save-account/email-confirmation dialogs, publish-website, item-properties, `new_context_menu_item`, `refresh_item_container`, `launch_app`, `item_icon`) plus the `puter` SDK resolve to `src/ezil-stubs.js`. 2026-08-02: removed one `window.hide_toolbar()` call (see the `MODIFIED BY EZIL` comment in-file; not previously recorded here). 2026-08-03: added one import (`telemetry.js`) and two `telemetry.capture()` calls, each one line beside a pre-existing `console.error` it does not replace — see `ezil/telemetry.js`'s header and this file's own `MODIFIED BY EZIL` comments at each site. 2026-08-04 (W2, converging minimise/close): `minimize_window` now tries `el_window._ezil_minimise?.()` first and returns if it handled the minimise — the hook EZiL's control drawer installs (`ezil/apps/desktop-window.js`) so this Puter-derived function stays ignorant of `ezil-fullbleed`/full-page state; falls through to the original body unchanged when absent. The fullpage-window-close branch in `close_one_window` now calls `window.exit_fullpage_mode(this)` instead of the previous no-argument call, so closing a full-bleed window actually reaches `UIDesktopFullpage.js`'s `el_window`-gated restore (chrome + geometry) instead of silently skipping it; the geometry half is itself skipped there when `data-closing === '1'`, which `$.fn.close` above already stamps synchronously, so a window mid-close-animation is not also jumped to a floating box first. Neither edit touches `is_dashboard_mode`/`pop_dashboard_app_url` — see this file's own note under `UIDesktopFullpage.js` below for why that subsystem was investigated and deliberately left in place. |
...
| `src/UI/UIDesktopFullpage.js` | 220 | `src/gui/src/UI/UIDesktop.js` ~L2405-2450 (of 2479) | `5a15719` | 2026-07-31; 2026-08-04 (see below) | Extraction of exactly three functions: `enter_fullpage_mode`, `exit_fullpage_mode`, `reset_window_size_and_position`. UIWindow calls all three by name, so they are a required seam. Inside them, `refresh_item_container()` and `refresh_desktop_background()` are dropped rather than stubbed — both sit on the restore path, and a rejecting stub there would strand a window in fullpage mode. The other ~2,430 lines of UIDesktop.js are not ported. **2026-08-04 (W2 item 4, EZiL-authored addition, not upstream):** `exit_fullpage_mode` split into a new `window.exit_fullpage_chrome(el_window)` (taskbar/head/body-class only, never touches geometry) plus the geometry half; `exit_fullpage_mode` itself now calls both, skipping the geometry half when `data-closing === '1'`. `enter_fullpage_mode` stashes the window's pre-fullpage `width/height/top/left/border-radius` on `el_window.dataset` (once — a second entry without an intervening exit does not overwrite it); `reset_window_size_and_position` restores from that stash and clears it, falling back to the historical hard-coded 680x380 centered box only when nothing was ever stashed. Purpose: kill the ~220ms flash to a wrong, hard-coded box on minimise-then-restore. **Minimise itself should call `exit_fullpage_chrome` alone** (no geometry churn at all while hidden) — that call site lives in `ezil/apps/desktop-window.js` (`minimise_to_taskbar` / the `_ezil_minimise` hook), owned by a different workstream; see that file's own history for whether it has been switched over. Until it is, minimise still goes through the combined `exit_fullpage_mode` and will show the window's real (now-correct, no longer hard-coded) pre-fullpage geometry for one frame instead of none. |

**Dashboard-URL subsystem (`is_dashboard_mode`, `push_dashboard_app_url`,
`pop_dashboard_app_url`, the `popstate` listener and its 400ms watchdog, all
in `UIWindow.js`) — investigated 2026-08-04, deliberately NOT removed.**
`is_dashboard_mode` is never assigned by any shipped EZiL code — confirmed by
`grep -rn "is_dashboard_mode\s*=" shell/` excluding `==` comparisons — so the
subsystem is dead in the running app, exactly as suspected. But that grep also
turns up the ONE place it IS assigned: `ezil/ui/Settings/stacking-browser-test.mjs`'s
"GAP 5" section (`runDashboardPopstateGuardSweep`), which deliberately flips
the flag to drive the real `push_dashboard_app_url`/`pop_dashboard_app_url`/
popstate handler/watchdog through their real code paths and asserts three
guards (M6/M7/M8) that a window mid-teardown (`data-closing === '1'`) is never
minimized or re-focused by a stale traversal. That test file's own header
explains why: the code is dead in the shipped app on purpose, but still real,
committed, reachable source, and a regression in one of those guards would
otherwise ship invisibly the day this fork's dashboard mode (or a future one)
turns the flag on. Removing the subsystem would require rewriting or deleting
that test — which lives under `shell/ezil/**`, owned by a different
workstream in this round, and is part of the `stacking-browser-test.mjs`
578/578 baseline this change is required to keep green. Left in place; flagged
for whoever next touches `ezil/ui/Settings/stacking-browser-test.mjs` or takes
ownership of the dashboard-URL code's fate.
