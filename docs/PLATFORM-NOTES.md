...
## 22. `sleepAfter` is not a timer — it is a *last-request* clock, and your own alarm resets it

`getSandbox(..., { sleepAfter: '30m' })` reads like "this container stops 30 minutes after it
boots." It does not. It stops 30 minutes after the last *request reaches the container*, and the
SDK counts your own bookkeeping as a request. Worse, `Sandbox.containerFetch()` calls
`startAndWaitForPorts` internally, so a container RPC does not merely reset the clock on a running
container — **it starts a stopped one.**

This project ran a workspace-flush loop on a self-rescheduling DO alarm every 10s
(`WORKSPACE_FLUSH_INTERVAL_SECONDS`), and each cycle walked the workspace over container RPCs
(`listFiles` recursive + per-file `readFile`). The result: `sleepAfter` could never fire, because
the flush *was* the activity keeping the container awake. Containers became immortal at first boot.

**Measured, not inferred.** Instance `guac-cc5e88bd…` was created `2026-08-03T11:35:57Z` and was
still `running` 26 hours later under a 30-minute sleep policy, with nobody attached. On
`standard-3` (2 vCPU / 8 GiB) that is 8 GiB x 26 h = 208 GiB-hours from one abandoned container —
about a quarter of that month's entire container-memory bill, at Cloudflare's $0.0000025/GiB-s
(= $0.009/GiB-hour, ~$52/month per permanently-idle container).

The billing signature is the giveaway, and it generalises: **memory-seconds measure residency,
vCPU-seconds measure work.** 3.03M GiB-s against only 50.4k vCPU-s meant ~105 container-hours
resident for ~7 container-hours of compute. When those two numbers diverge by an order of
magnitude, nothing is "running too hard" — something is refusing to stop.

Half of this was already known here and applied too narrowly: a tombstone (`WORKSPACE_TERMINATED_KEY`)
had been added precisely because the flush alarm was *resurrecting explicitly destroyed* containers.
The same reasoning was never extended to the ordinary case of a container going quiet, and a
`containerIsRunning()` helper sat in the file called only from teardown paths.

Consequences worth keeping:

- **An alarm that touches a container must first ask whether a container should exist.** Check
  `ctx.container?.running` *before* any RPC, and do not reschedule when it is false — let the next
  real boot restart the loop.
- **Never let a liveness signal be generated by the thing whose liveness it reports.** Derive
  activity from the user (here: `lastActivityAt = now - lastInputAgoMs` from a client heartbeat, so
  a heartbeat arriving 59s after the last keystroke does not read as fresh input), and never bump it
  from the alarm — an alarm that marks itself as activity rebuilds immortality one layer down.
- **Stop explicitly; do not trust the platform timer.** `sleepAfter` is a backstop, not a mechanism.
  Flush first, and stop *only if that flush succeeded* — losing a user's files costs more than the
  container ever will.
- **Idle-stop is not termination.** Reusing the terminate tombstone for it would mean an
  idle-stopped computer never boots again.
